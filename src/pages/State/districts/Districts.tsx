...
export default function StateDistricts() {
  ...
  const [showDistrictsWithoutUsers, setShowDistrictsWithoutUsers] =
    useState(false);
  ...
  const transformedData: HierarchyChild[] = data.map(
    (district: DistrictChild) => ({
  ...
  return (
    <div className="p-2 bg-[var(--bg-main)] min-h-screen">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="shrink-0">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">
              {levelLabels.districtLabel} list
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
            ...
            <div className="bg-[var(--bg-card)] text-[var(--text-color)] rounded-md shadow-md p-3 flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-[var(--text-secondary)]">
                  Total {levelLabels.districtLabel}
                </p>
            ...
            <div
              ...
              title={
                totalUsers > 0
                  ? "Click to view districts with assigned users"
                  : "No assigned users"
              }
            >
              <div>
                ...
                <p
                  className={`text-xl sm:text-2xl font-semibold mt-1 ${totalUsers > 0 ? "text-green-600" : "text-[var(--text-secondary)]"}`}
                >
                  {formatNumber(totalUsers)}
                </p>
              </div>
              <div
                className={`rounded-full p-1.5 ${totalUsers > 0 ? "bg-green-50" : "bg-[var(--bg-main)]"}`}
              >
                <svg
            ...
            <div
              ...
              className={`bg-[var(--bg-card)] text-[var(--text-color)] rounded-md shadow-md p-3 flex items-center justify-between transition-all duration-200 ${
                ...
                showDistrictsWithoutUsers ? "ring-2 ring-red-500 bg-red-50" : ""
              }`}
              title={
                districtsWithoutUsers > 0
                  ? "Click to view districts without users"
                  : "No districts without users"
              }
            >
              <div>
                <p className="text-xs font-medium text-[var(--text-secondary)]">
                  {levelLabels.districtLabel} without users
                  {showDistrictsWithoutUsers && (
                ...
                <p
                  className={`text-xl sm:text-2xl font-semibold mt-1 ${districtsWithoutUsers > 0 ? "text-red-600" : "text-[var(--text-secondary)]"}`}
                >
                  {formatNumber(districtsWithoutUsers)}
                </p>
              </div>
              <div
                className={`rounded-full p-1.5 ${districtsWithoutUsers > 0 ? "bg-red-50" : "bg-[var(--bg-main)]"}`}
              >
                {districtsWithoutUsers > 0 ? (
                  <svg
      ...
      <HierarchyTable
        ...
        stateLabel={levelLabels.stateLabel}
        districtLabel={levelLabels.districtLabel}
        assemblyLabel={levelLabels.assemblyLabel}
      />
    </div>
  );
}
