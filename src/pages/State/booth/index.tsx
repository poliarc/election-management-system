...
const STORAGE_KEY = "boothCandidates";

export default function StateBoothListing() {
  const [rows, setRows] = useState<BoothCandidate[]>([]);
  ...
  const selectedBoothNo = watch("boothNo");
  const q = watch("q");
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        const normalized: BoothCandidate[] = parsed.map((r, idx) => ({
          boothNo: (r["boothNo"] as number | string) ?? idx + 1,
          designation:
          ...
          blockName:
            ...
            undefined,
          mandalName:
            (r["mandalName"] as string | undefined) ||
            (r["mandal"] as string) ||
            undefined,
        }));
        setRows(normalized);
  ...
  const boothOptions = useMemo(() => {
    ...
    rows
      ...
      .filter((r) =>
        selectedMandalName ? r.mandalName === selectedMandalName : true
      )
      .forEach((r) => r.boothNo && set.add(r.boothNo.toString()));
    return Array.from(set).sort();
  }, [
    rows,
    selectedState,
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const matchMandalName = selectedMandalName
        ...
        : true;
      const matchBoothNo = selectedBoothNo
        ? r.boothNo?.toString() === selectedBoothNo
        : true;
      const hay = [
        r.boothNo?.toString(),
        r.designation,
  ...
  const handleDelete = (idx: number) => {
    if (!confirm("Delete this booth candidate?")) return;
    const updated = [...rows];
      ...
      <form className="mb-4 w-full flex flex-col sm:flex-row sm:items-end gap-3 flex-wrap">
        ...
        <div className="flex flex-col gap-1 sm:flex-1 min-w-[220px]">
          ...
          <input
            id="q"
            type="text"
            placeholder="Search booth no, designation, name..."
      ...
      <div className="overflow-x-auto w-full">
        <table className="w-full text-sm text-left rounded-lg shadow-md overflow-hidden bg-white">
          <thead className="bg-indigo-50 text-[13px]">
            <tr>
              <th className="px-4 py-2 font-semibold">BoothNo</th>
          ...
          <tbody>
            {shown.length === 0 ? (
              ...
              shown.map((c, index) => (
                ...
                >
                  <td className="px-4 py-2">{c.boothNo ?? index + 1}</td>
