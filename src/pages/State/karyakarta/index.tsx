...
const STORAGE_KEY = "karyakartaCandidates";

export default function StateKaryakartaListing() {
  const [rows, setRows] = useState<KaryakartaCandidate[]>([]);
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        const normalized: KaryakartaCandidate[] = parsed.map((r) => ({
          firstName:
            (r["firstName"] as string) ||
            (r["name"] as string)?.split(" ")[0] ||
            "",
          lastName:
            (r["lastName"] as string) ||
            (r["name"] as string)?.split(" ")[1] ||
            "",
  ...
  const handleDelete = (idx: number) => {
    if (!confirm("Delete this karyakarta?")) return;
    const updated = [...rows];
