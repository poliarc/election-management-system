...
export const MarkedVoter: React.FC = () => {
  const { t } = useTranslation();
  
  // NEW: Grab levelId from URL
  ...
  const currentDistrictId = levelUsersData?.level?.assemblyParentId;
  const currentAssemblyId = levelUsersData?.level?.assembly_id;
  ...
  // Fetch data with full hierarchy
  ...
  // 🔥 FIX 2: Only show the error if BOTH IDs are missing
  if (!currentAssemblyId && !currentAfterAssemblyId) {
    return (
        <div className="p-6">
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded">
                No level selected. Please select an assembly or sublevel first.
            </div>
        </div>
    );
  }
