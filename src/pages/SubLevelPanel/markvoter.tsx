...
export const SubLevelMarkedVoter: React.FC = () => {
  console.log("🚨🚨🚨 MARK VOTER COMPONENT IS RENDERED! 🚨🚨🚨");
  ...
  // AGGRESSIVE FALLBACKS: If Redux is empty (e.g. on refresh), grab from LocalStorage
  let authUser = {};
  try {
      authUser = JSON.parse(localStorage.getItem('auth_user') || '{}');
  } catch(e) {}

  const currentUserId = user?.user_id || user?.id || (authUser as any)?.id || (authUser as any)?.user_id;
  ...
  const { 
    ...
    isFetching: isCurrentLevelFetching 
  } = useGetAfterAssemblyLevelUsersQuery(currentAfterAssemblyId!, {
    skip: !currentAfterAssemblyId,
  });

  const currentLevel = currentLevelData?.data?.level || currentLevelData?.level;

  const isMissingAssemblyContext = currentLevel && (!currentLevel.assembly_id && !currentLevel.parentAssemblyId);
  const needsParentContext = Boolean(isMissingAssemblyContext && currentLevel.parentId);

  const { 
    data: parentLevelData, 
    isFetching: isParentLevelFetching 
  } = useGetAfterAssemblyLevelUsersQuery(currentLevel?.parentId, {
    skip: !needsParentContext,
  });

  const parentLevel = parentLevelData?.data?.level || parentLevelData?.level;

  const resolvedAssemblyId =
    currentLevel?.assembly_id || 
    currentLevel?.parentAssemblyId || 
    parentLevel?.assembly_id ||
    parentLevel?.parentAssemblyId;

  const resolvedDistrictId =
    currentLevel?.assemblyParentId || 
    parentLevel?.assemblyParentId || 
    currentLevel?.district_id ||  
    parentLevel?.district_id;

  const isResolvingContext = isCurrentLevelFetching || (needsParentContext && isParentLevelFetching);

  // RELAXED GATEKEEPER: Only requires that we are done loading, and we have the Level ID + User ID
  const isContextReady = Boolean(
    !isResolvingContext && 
    currentAfterAssemblyId &&
    currentUserId
  );

  // DEBUG LOGGER (You can remove this later once it's working perfectly)
  useEffect(() => {
    console.log("%c--- GATEKEEPER STATUS ---", "color: orange; font-weight: bold; font-size: 14px;");
    console.log("1. Resolving Context (Loading)? :", isResolvingContext);
    console.log("2. Assembly ID (Found)          :", resolvedAssemblyId);
    console.log("3. District ID (Found)          :", resolvedDistrictId);
    console.log("4. After Assembly ID (levelId)  :", currentAfterAssemblyId);
    console.log("5. State ID                     :", currentStateId);
    console.log("6. User ID                      :", currentUserId);
    console.log(`%cIS API READY TO FIRE? -> ${isContextReady}`, `color: ${isContextReady ? 'lime' : 'red'}; font-weight: bold; font-size: 14px;`);
  }, [isResolvingContext, resolvedAssemblyId, resolvedDistrictId, currentAfterAssemblyId, currentStateId, currentUserId, isContextReady]);

  // =======================================================================
  // MAIN VOTER DATA LOGIC
  // =======================================================================
  ...
  // 1. Loading UI: Prevents flicker while resolving parent IDs
  if (isResolvingContext) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[400px] text-[var(--text-secondary)]">
        <Loader2 className="w-8 h-8 animate-spin text-indigo-600 mb-4" />
        <p>Resolving location hierarchy...</p>
      </div>
    );
  }

  // NOTE: The strict `if (!resolvedAssemblyId)` block was completely removed here!
...
export default SubLevelMarkedVoter;