...
export default function SubLevelAssignUser() {
  ...
  const getStateId = () => {
    ...
    if (user?.state_id) {
      console.log("Using state_id from Redux user:", user.state_id);
      return user.state_id;
    }

    // Try to get state_id directly from auth_user in localStorage
    try {
      const authUser = localStorage.getItem("auth_user");
      if (authUser) {
        const parsed = JSON.parse(authUser);
        if (parsed?.state_id) {
          console.log("Using state_id from auth_user localStorage:", parsed.state_id);
