...
export default function UserManagement() {
  ...
  useEffect(() => {
    ...
    loadUsers();
  }, [currentPanel, currentPage, searchTerm]);
  ...
  useEffect(() => {
    ...
    loadAssignedUsers();
  }, [currentPanel, assignedPage, searchTerm]);
  ...
  const handleAssignUser = async (userId: number) => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(
          `User assigned to ${currentPanel.metadata.stateName} successfully`
        );
        ...
        if (updatedResponse.success) {
          const usersData =
            updatedResponse.data?.users || updatedResponse.data || [];
          const filteredUsers = usersData.filter(
            (user: AssignedUser) => user.isSuperAdmin !== 1
          );
          setAssignedUsers(filteredUsers);
        ...
        if (availableResponse.success) {
          // Filter out super admin users
          const filteredUsers = availableResponse.data.filter(
            (user: User) => !user.isSuperAdmin
          );
          setAllUsers(filteredUsers);
      ...
      if (response.success) {
        ...
        if (updatedResponse.success) {
          ...
          if (updatedResponse.pagination) {
            setAssignedTotalPages(updatedResponse.pagination.totalPages);
            setAssignedTotalCount(
              updatedResponse.pagination.total || filteredUsers.length
            );
          } else {
            setAssignedTotalCount(filteredUsers.length);
          }
