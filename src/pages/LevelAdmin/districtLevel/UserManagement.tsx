...
export default function DistrictUserManagement() {
    ...
    const handleAssignUser = async (userId: number) => {
        ...
        try {
            ...
            const response = await assignUserToState(userId, selectedDistrict.location_id);

            if (response.success) {
                toast.success(`User assigned to ${selectedDistrict.location_name} successfully`);

                // Refresh districts
                const updatedResponse = await fetchHierarchyChildren(
                    metadata.stateId,
                    {
                        page: 1,
                        limit: 1000
    ...
    const openUnassignModal = (userId: number, userName: string, districtId: number) => {
        setConfirmModal({
            isOpen: true,
            userId,
            userName,
    ...
    const handleUnassignUser = async () => {
        const metadata = currentPanel?.metadata;
        if (!confirmModal.userId || !confirmModal.districtId || !metadata?.stateId) return;
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600 mt-1">
                                                    {district.total_users} users ({district.active_users} active)
                                                </p>
                                            </div>
                                        </div>
                                    </button>
                        ...
                        <div className="bg-white rounded-lg shadow-md">
                            <div className="p-4 border-b border-gray-200">
                                <div className="flex justify-between items-center">
                                    <h2 className="text-lg font-bold text-gray-800">{selectedDistrict.location_name}</h2>
                                    <button
                                            ...
                                            {filteredDistrictUsers.length === 0 ? (
                                                <tr>
                                                    <td colSpan={6} className="px-6 py-12 text-center">
                                                        <p className="text-gray-500">No users assigned to this district</p>
                                                    </td>
                                                </tr>
                                            ) : (
                                                filteredDistrictUsers.map((user, index) => (
