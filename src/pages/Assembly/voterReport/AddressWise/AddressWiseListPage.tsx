            ...
            <div className="mb-1 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--text-color)]">
                        {t("AddressWiseListPage.Title")}
                    </h1>
                    <p className="text-[var(--text-secondary)] mt-1">
                        {t("AddressWiseListPage.Desc")}
                    </p>
                </div>
                ...
                <>
                    <div className="bg-[var(--bg-card)] p-1 rounded-lg shadow mb-1">
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">
                                    {t("AddressWiseListPage.Part_No_From")}
                                </label>
                                <input
                                    type="number"
                                    value={partFrom || ""}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">
                                    {t("AddressWiseListPage.Part_No_To")}
                                </label>
                                <input
                                    type="number"
                                    value={partTo || ""}
                            ...
                            <div>
                                ...
                                <button
                                    onClick={handleReset}
                                    className="w-full bg-[var(--bg-color)] 0 text-[var(--text-secondary)] px-4 py-2 rounded-lg hover:bg-gray-200 transition"
                                >
                                    {t("AddressWiseListPage.Reset")}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {isLoading ? (
                        <div className="text-center py-8">
                            <div className="text-[var(--text-secondary)]">{t("AddressWiseListPage.Loading")}</div>
                        </div>
                    ) : (
                        <>
                            <div className="mb-1 text-sm text-[var(--text-secondary)] bg-blue-50 p-3 rounded-lg border border-blue-200">
                                {t("AddressWiseListPage.Found")} {filteredVoters.length} {t("AddressWiseListPage.voters")}
                                {searchAddress && <span> • {t("AddressWiseListPage.Address")} "{searchAddress}"</span>}
                                {(partFrom || partTo) && (
                                    <span> • {t("AddressWiseListPage.Part_No")} {partFrom || "any"} - {partTo || "any"}</span>
                                )}
                            </div>
                            <VoterListTable
                            ...
                            {totalPages > 1 && (
                                <div className="mt-6 flex items-center justify-between bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-color)]">
                                    <div className="text-sm text-[var(--text-secondary)]">
                                        {t("AddressWiseListPage.Showing_page")} {page} {t("AddressWiseListPage.of")} {totalPages} • {totalVoters.toLocaleString()} {t("AddressWiseListPage.total_voters")}
                                    </div>
                                    <div className="flex gap-2">
                                        ...
                                        >
                                            {t("AddressWiseListPage.Previous")}
                                        </button>
                                        <button
                                            onClick={() => setPage(page + 1)}
