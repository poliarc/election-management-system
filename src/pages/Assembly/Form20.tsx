...
export default function AssemblyForm20() {
  ...
  const [boothSearchTerm, setBoothSearchTerm] = useState("");
  ...
  const filteredBooths = useMemo(() => {
    if (!boothSearchTerm.trim()) {
      return boothWiseResults;
    }
    return boothWiseResults.filter(booth =>
      booth.boothNo.toLowerCase().includes(boothSearchTerm.toLowerCase())
    );
  }, [boothWiseResults, boothSearchTerm]);
