...
import { useGetBlockHierarchyQuery, useGetBlockTeamQuery } from '../../store/api/blockTeamApi';
...
export default function BlockDashboard() {
    ...
    const blockName = selectedAssignment?.displayName || selectedAssignment?.levelName || "Block";

    // Fetch block hierarchy (mandals)
    const { data: hierarchyData } = useGetBlockHierarchyQuery(levelId!, {
        skip: !levelId,
    });

    // Fetch block team (users)
    const { data: teamData } = useGetBlockTeamQuery(levelId!, {
        skip: !levelId,
    });

    const totalMandals = hierarchyData?.pagination.total || 0;
    const totalUsers = teamData?.total_users || 0;
    const activeUsers = teamData?.users.filter(u => u.user_active === 1).length || 0;
    ...
    const navCards: NavCard[] = useMemo(
        ...
        [totalMandals, totalUsers, activeUsers, blockName]
    );
