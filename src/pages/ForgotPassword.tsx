...
// Phone number validation for password reset
...
export default function ForgotPasswordPage() {
  ...
  const previousStepRef = useRef<string>("entry");
  ...
  // Toast notifications for step changes
  useEffect(() => {
    const previousStep = previousStepRef.current;

    if (step === "otp" && previousStep === "entry" && maskedIdentifier) {
      // Initial OTP send
      toast.success(`OTP sent successfully to ${maskedIdentifier}`);
    } else if (
      step === "otp" &&
      previousStep === "otp" &&
      resendCooldown === 30
    ) {
      // Resend OTP (step stays the same but cooldown resets)
      toast.success("OTP resent successfully");
    } else if (step === "reset" && previousStep === "otp") {
  ...
  }, [step, maskedIdentifier, resendCooldown]);
