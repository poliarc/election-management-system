...
function GroupInfoModal({ group, onClose }: GroupInfoModalProps) {
  ...
  const groupCreatorId =
    typeof group.created_by === "string"
      ? parseInt(group.created_by)
      : group.created_by;
          ...
          <div className="space-y-2">
            {members.length === 0 ? (
              ...
              members.map((member: any) => {
                ...
                const isActive =
                  member.is_active?.data?.[0] === 1 || member.is_active === 1;
...
function ChatPopup({ chat, onClose, position }: ChatPopupProps) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (newMessage: any) => {
      console.log(
        "[ChatPopup] Received socket message:",
        newMessage,
        "for chat:",
        chat.id
      );
      ...
      if (isGroup) {
        ...
        // Direct message
        if (
          (newMessage.sender_id === chat.id &&
            newMessage.receiver_id === currentUserId) ||
          (newMessage.sender_id === currentUserId &&
            newMessage.receiver_id === chat.id)
        ) {
  ...
  useEffect(() => {
    ...
    socketService.on(
      isGroup ? "group_stop_typing" : "stop_typing",
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setLocalMessages((prev) =>
        prev.filter((msg) => (msg.chat_id || msg.Id) !== tempId)
      );
    } catch (error) {
  ...
  const positionStyle = useMemo(() => {
    ...
    if (isMobile)
      return { right: "0px", bottom: position === 0 ? "60px" : "520px" };
...
export function ChatUsersList({ onSelectUser, openChats, isMobileModal = false }: ChatUsersListProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [isExpanded, setIsExpanded] = useState(isMobileModal ? true : false); // Always expanded on mobile modal
          ...
          <>
            <div className="p-3 border-b border-gray-200 space-y-2">
              ...
              <button
                ...
                className="w-full px-3 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium flex items-center justify-center gap-2"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              {activeTab === "recent" ? (
                ...
                filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((u) => {
                    ...
                    const initial = fullName.charAt(0).toUpperCase() || "U";
                    const isOpen = openChats.some(
                      (chat) => chat.type === "direct" && chat.id === u.user_id
                    );
                    return (
                      <div
                        key={u.user_id}
