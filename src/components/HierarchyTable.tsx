...
interface HierarchyTableProps {
    ...
    onSort: (sortBy: 'location_name' | 'total_users' | 'active_users', order: 'asc' | 'desc') => void;
...
export default function HierarchyTable({
    ...
    const [selectedUsers, setSelectedUsers] = useState<{ users: any[]; locationName: string } | null>(null);
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="mt-3 flex items-center justify-between text-sm text-gray-600">
                    {searchInput && (
                        <span className="text-blue-600">Searching for: "{searchInput}"</span>
                    )}
                </div>
            </div>
                            ...
                            {data.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="px-6 py-12 text-center">
                                        ...
                                        <p className="mt-2 text-gray-500 font-medium">{emptyMessage}</p>
                                    </td>
                                </tr>
                            ) : (
                                data.map((item, index) => (
                                    <tr key={item.location_id} className="hover:bg-blue-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                ...
                                                <span className="text-sm font-medium text-gray-900">{item.total_users}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="w-2 h-2 bg-green-500 rounded-full mr-2"></div>
                                                <span className="text-sm font-medium text-green-600">{item.active_users}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <button
                                                onClick={() => {
                                                    console.log('View clicked:', item.location_name, 'Users count:', item.users?.length || 0);
                                                    setSelectedUsers({ users: item.users || [], locationName: item.location_name });
                                                }}
                                                ...
                                                title={item.users && item.users.length > 0 ? 'View Users' : 'No users assigned'}
                                            >
