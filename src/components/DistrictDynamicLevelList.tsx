...
export default function DistrictDynamicLevelList({
    levelName,
    displayLevelName }: DistrictDynamicLevelListProps) {
    ...
    // State for filter data - District context (no district filter needed)
    ...
    useEffect(() => {
        if (selectedAssignment) {
            setDistrictInfo({
                districtName: selectedAssignment.levelName || selectedAssignment.displayName || "",
    ...
    const getVisibleFilters = () => {
        ...
        if (currentLevelIndex === -1) return hierarchyOrder.slice(2, 6); // Start from Assembly (skip State, District)

        // Show filters from Assembly up to (but not including) the current level
        return hierarchyOrder.slice(2, currentLevelIndex); // Skip State and District
    };
    ...
    // Get current level items based on filters - District context
    ...
    // Fetch all items for the current level - District context
    useEffect(() => {
        const fetchAllLevelItems = async () => {
            ...
            try {
                ...
                // Step 1: Fetch all assemblies for the current district
                const assemblies = await fetchAllPages(
                    `${import.meta.env.VITE_API_BASE_URL}/api/user-state-hierarchies/hierarchy/children/${districtInfo.districtId}`
                );

                // Step 2: Fetch all after-assembly levels for each assembly (flexible approach)
        ...
        fetchAllLevelItems();
    }, [districtInfo.districtId, levelName]);
                        ...
                        <div className={`grid grid-cols-1 md:grid-cols-3 lg:grid-cols-${Math.min(visibleFilters.length + 3, 8)} gap-4`}>
                            ...
                            <div className="flex items-end">
                                <button
                                    ...
                                    disabled={!selectedAssemblyId && !selectedBlockId && !selectedMandalId && !selectedPollingCenterId && !searchTerm && !selectedLevelFilter}
