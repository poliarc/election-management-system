...
export default function LevelAdminSidebar({ onNavigate }: LevelAdminSidebarProps) {
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      const panel = levelAdminPanels.find((p: any) => p.id === panelId);
      ...
      const panelFromLocation = locationState?.levelId
        ? levelAdminPanels.find((p: any) => p.id === locationState.levelId)
        : null;

      const panelFromAssignment = selectedAssignment?.levelType
        ? levelAdminPanels.find(
            (p: any) =>
              p.name?.toLowerCase() === selectedAssignment.levelType.toLowerCase()
          )
        : null;

      const assemblyPanel = levelAdminPanels.find(
        (p: any) => p.name?.toLowerCase() === "assembly"
      );

      setSelectedPanel(
        panelFromLocation || panelFromAssignment || assemblyPanel || levelAdminPanels[0]
      );
    } else {
  ...
  const isBoothLevel = panelNameLower === "booth";

  const parentLevelType =
    selectedPanel.parentLevelType ||
    selectedPanel.parentLevelName ||
    selectedPanel.metadata?.parentLevelType ||
    selectedPanel.metadata?.parentLevelName ||
    "";

  const parentLevelLower = parentLevelType.toLowerCase();

  /**
   * WhatsApp visibility rule:
   *
   * 1. Show WhatsApp on Assembly level.
   * 2. Show WhatsApp on the level directly after Assembly.
   *
   * Example:
   * Assembly -> Mandal -> Booth -> Polling Center
   *
   * Show:
   * - Assembly
   * - Mandal
   ...
   * - Polling Center
   */
  const isAssemblyLevel = panelNameLower === "assembly";
  const isDirectChildOfAssembly = parentLevelLower === "assembly";

  const showWhatsApp = isAssemblyLevel || isDirectChildOfAssembly;

  /**
   * Route rule:
   *
   * Assembly WhatsApp page:
   * /whatsapp
   *
   * Level just after Assembly WhatsApp page:
   * /mandal-whatsapp
   *
   * Note:
   * Even if the level after Assembly is not called Mandal,
   * it will still use /mandal-whatsapp because this is your existing second WhatsApp route.
   */
  const whatsappRoute = isAssemblyLevel ? "/whatsapp" : "/mandal-whatsapp";
  ...
  const navItems = [
    ...
    },
    {
      ...
      label:
        selectedPanel.name !== "State" &&
        selectedPanel.name !== "District" &&
        selectedPanel.name !== "Assembly"
          ? "Manage Levels"
          : "User Management",
    },
    ...(isBoothLevel
