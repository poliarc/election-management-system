  ...
  endpoints: (builder) => ({
    ...
    >({
      query: ({ partyId, stateId, page = 1, limit = 20, search = "" }) => {
        ...
        if (!stateId) {
          console.warn(
            "API Call: state_id is missing from /users/filter call. This may limit user filtering.",
          );
        }
      ...
      transformResponse: (
        response: ApiResponse<UserByParty[]> & { pagination?: any },
      ) => {
    ...
    getAssemblyByState: builder.query<
      {
        success: boolean;
        message: string;
        data: { stateHierarchy: Assembly[]; afterAssemblyHierarchy: any[] };
      },
      { state_id: number; party_id: number }
    >({
      query: ({ state_id, party_id }) =>
        `/campaigns/hierarchy?state_id=${state_id}&party_id=${party_id}`,
    ...
    >({
      query: ({ assemblyId, page = 1, limit = 20, search = "" }) => {
        let url = `/v2/Dash/usersbyassembely/${assemblyId}?page=${page}&limit=${limit}`;
      ...
      transformResponse: (response: any) => {
        ...
        throw new Error(
          response.message || "Failed to fetch users by assembly",
        );
      },
...
export const {
  ...
  useLazyGetUsersByAssemblyIdQuery,
